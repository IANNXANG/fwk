...
# 创建输出目录
output_dir = "problem_visualizations_umap"
...
def main():
    ...
    print("绘制TF-IDF特征的UMAP可视化...")
    plot_umap_results(
        umap_results_tfidf, 
        labels, 
        "基于TF-IDF特征的问题文本UMAP可视化", 
        os.path.join(output_dir, "tfidf_umap_visualization.png")
    )
    
    # 2. 使用BERT特征进行UMAP分析（如果可用）
    bert_features = extract_features_bert(all_problems)
    if bert_features is not None:
        print("应用UMAP降维到BERT特征...")
        # 使用不同的参数获得不同的可视化效果
        umap_results_bert = apply_umap(bert_features, n_neighbors=30, min_dist=0.3)
        
        print("绘制BERT特征的UMAP可视化...")
        plot_umap_results(
            umap_results_bert, 
            labels, 
            "基于BERT特征的问题文本UMAP可视化", 
            os.path.join(output_dir, "bert_umap_visualization.png")
        )

        # 额外尝试一组不同的UMAP参数
        print("使用不同参数应用UMAP降维到BERT特征...")
        umap_results_bert_alt = apply_umap(bert_features, n_neighbors=50, min_dist=0.1)
        
        print("绘制另一组参数的BERT特征UMAP可视化...")
        plot_umap_results(
            umap_results_bert_alt, 
            labels, 
            "基于BERT特征的问题文本UMAP可视化 (n_neighbors=50)", 
            os.path.join(output_dir, "bert_umap_alt_visualization.png")
        )
    ...
    print("所有分析完成，可视化结果保存在 'problem_visualizations_umap' 目录中")
